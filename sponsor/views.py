...
from client.models import (UserToken, CampaignNotifier, 
...
class DashboardView(SponsorMixin, TemplateView):
    ...
    def render_to_response(self, context, **response_kwargs):
        ...
        notification_tracker_obj = NotificationTracker.objects.filter(
            sponsor_id=sponsor_id
        )
        notify_type="coupon"
        ...
        context['brand_reminder'] = notification_tracker_obj.filter(
        	notify_type="coupon"
        ).count()
        context['twitter_shares'] = SocialShare.objects.filter(
            sponsor_id=sponsor_id,
            social_type="twitter"
        ).count()
        context['future_rewards'] = notification_tracker_obj.filter(
...
class CampaignView(SponsorMixin, TemplateView):
    ...
    def render_to_response(self, context, **response_kwargs):
        ...
        context['campaign_list'] = [{
            ...
            'image': data.image,
            'budget': (data.coupons_available)*5,
...
class ReportView(SponsorMixin, TemplateView):
	...
	def render_to_response(self, context, **response_kwargs):
		...
		if campaign_id:
			tracker_obj = CampaignTracker.objects.filter(
				sponsor_id=user_id, campaign_id=campaign_id
			)
			notification_tracker_obj = NotificationTracker.objects.filter(
				sponsor_id=user_id, 
			...
			context['brand_view'] = tracker_obj.filter(action='vd').count()
			context['brand_click'] = tracker_obj.filter(action='cd').count()
			context['redeemed'] = UserToken.objects.filter(
				is_redeemed='true',
				campaign_id=campaign_id
			).count()
			context['brand_reminder'] = notification_tracker_obj.filter(
		...
		context['campaign_list'] = [{
			'campaign_name': data.campaign_name,
			'id': data.pk,
			'selected': True if data.pk == campaign_id else False
		} for data in all_campaign]
