...
class PublisherWebsiteForm(WebsiteForm):
	...
	def __init__(self, *args, **kwargs):
		...
		self.fields['website_name'].widget = forms.TextInput(attrs={
            'type':'text', 'placeholder':'Enter website name',
            'class': 'form-control input-small', 'style': 'height: 25px'
    	})
		self.fields['website_domain'].widget = forms.TextInput(attrs={
            'type':'url', 'placeholder':'Enter your website domain',
            'class': 'form-control input-small', 'style': 'height: 25px'
    	})
...
class AddwebsiteForm(forms.ModelForm):
	...
	name = forms.CharField(
        widget=forms.TextInput(attrs={
            'type':'text', 'placeholder':'Enter website name',
            'class': 'span input-xlarge', "style" : "height:15px",
            'required': "", 
    }))
	domain = forms.URLField(initial='http://',
        widget=forms.TextInput(attrs={
            'type':'url', 'placeholder':'Enter your website domain',
            'class': 'span input-xlarge', "style" : "height:15px",
            'required': "", 
